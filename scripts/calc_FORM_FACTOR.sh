...
cat electronDENSITYcent.xvg | awk -v slice=$slice-v bulkDENS=$bulkDENS 'BEGIN{scale=0.01;}{for(q=0;q<1000;q=q+1){Fa[q]=Fa[q]+($2-bulkDENS)*cos(scale*q*$1)*slice;Fb[q]=Fb[q]+($2-bulkDENS)*sin(scale*q*$1)*slice}}END{for(q=0;q<1000;q=q+1){print 0.1*q*scale" "sqrt(Fa[q]*Fa[q]+Fb[q]*Fb[q])
